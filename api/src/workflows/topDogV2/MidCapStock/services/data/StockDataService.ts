...
export class StockDataService {
  ...
  async getStockData(ticker: string, date: string): Promise<Stock> {
    ...
    const [
      ...
      this.tradierClient.getPreMarketData(ticker, `${date} 04:00`, `${date} 09:30`),
      ...
      this.eodhdClient.getEarnings(ticker, yesterday, date),
      this.eodhdClient.getNews(ticker, 5, true)
    ]);
    
    // Step 3: Process the data into our standard format
    // Process price data
    const priceData = this.processPriceData(quoteData, preMarketData, historicalData, sma20Data, sma50Data);
    
    // Process volume data
    const volumeData = this.processVolumeData(quoteData, preMarketData, historicalData);
    
    // Process technical indicators
    const technicalIndicators = this.processTechnicalIndicators(
      rsiData, macdData, atrData, bollingerBands, adxData, patternData
    );
    
    // Process options data
    const optionsDataProcessed = this.processOptionsData(optionsData?.options?.option || []);
    ...
    const dayTradingMetrics = this.calculateDayTradingMetrics(
      priceData, technicalIndicators, historicalData
    );
  ...
  private processPriceData(
    ...
    // Extract pre-market price
    const preMarketSeries = preMarketData?.series?.data || [];
    const preMarketPrice = preMarketSeries.length > 0 
      ? preMarketSeries[preMarketSeries.length - 1].price 
      : quoteData.open;
    ...
    const firstHourData = historicalData?.history?.day 
      ? historicalData.history.day.filter((d: any) => {
          ...
          return dataDate.getDate() === day.getDate() &&
                 dataDate.getHours() >= 9 && dataDate.getHours() <= 10;
        })
      : [];
    
    const openingHighLow = firstHourData.length > 0
      ? {
    ...
    if (historicalData?.history?.day) {
      ...
      if (todayData.length > 0) {
        ...
        const volumeSum = todayData.reduce((sum: number, d: any) => sum + d.volume, 0);
    ...
    const ma20 = sma20Data?.data?.length > 0 
      ? sma20Data.data[sma20Data.data.length - 1].sma 
      : 0;
    
    const ma50 = sma50Data?.data?.length > 0 
  ...
  private processVolumeData(
    ...
    // Extract pre-market volume
    const preMarketSeries = preMarketData?.series?.data || [];
    const preMarketVolume = preMarketSeries.reduce((sum: number, d: any) => sum + (d.volume || 0), 0);
    ...
    const firstHourVolume = firstHourData.reduce((sum: number, d: any) => sum + (d.volume || 0), 0);
    const totalVolume = quoteData.volume || 0;
    
    const firstHourPercent = totalVolume > 0 
      ? Math.round((firstHourVolume / totalVolume) * 100)
      : 0;
    ...
    const relativeVolume = avg10DayVolume > 0 
      ? parseFloat((totalVolume / avg10DayVolume).toFixed(1))
      : 1.0;
    
    return {
      pre_market: preMarketVolume || 0,
      current: totalVolume,
  ...
  private processTechnicalIndicators(
    ...
    const latestBollingerBands = bollingerBands?.data?.length > 0 
      ? {
    ...
    const bbWidth = latestBollingerBands.middle > 0 
      ? parseFloat(((latestBollingerBands.upper - latestBollingerBands.lower) / latestBollingerBands.middle).toFixed(2))
      : 0;
    ...
    const bullishPatterns = patterns
      .filter((p: any) => p.strength >= 5 && ['bullish', 'both'].includes(p.trade_signal))
      .map((p: any) => p.pattern);
    
    const bearishPatterns = patterns
      .filter((p: any) => p.strength >= 5 && ['bearish', 'both'].includes(p.trade_signal))
      .map((p: any) => p.pattern);
  ...
  private processEarningsData(earningsData: any): EarningsData {
    const latestEarnings = Array.isArray(earningsData) && earningsData.length > 0 
      ? earningsData[0]
      : null;
  ...
  private processNewsData(newsData: any): NewsData {
    ...
    const hasPositiveNews = recentArticles.some(article => 
      article.sentiment && article.sentiment.score > 0.6
    );
    
    // Check for negative news
    const hasNegativeNews = recentArticles.some(article => 
  ...
  private calculateDayTradingMetrics(
    ...
    const volatilityScore = this.scoringService.calculateVolatilityScore(
      ...
      priceData.current,
      histPrices,
      priceData.current, // Using currentPrice as placeholder since we need avgVolume and currentVolume
      priceData.current  // This should be replaced with actual volume data
    );
